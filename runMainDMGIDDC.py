...
if __name__ == '__main__':
    ...
    d=['Reuters'] #['Reuters','yale_mtv','MSRCv1','3sources','small_Reuters','small_NUS','BBC','BBCSport'，‘LandUse-21’] # ['BBCSport','yale_mtv','MSRCv1','3sources']
    ...
    for data in d:
        for link in ['Mean']:
            ...
            # input arguments
            parser = argparse.ArgumentParser(description='DMGIDDC')
            parser.add_argument('--embedder', nargs='?', default='DMGIDDC')
            ...
            parser.add_argument('--cluster_hidden_dim', type=int, default=100, help='cluster_hidden_dim')
            parser.add_argument('--l2_coef', type=float, default=config[data]['l2_coef'][0], help='l2_coef')
            parser.add_argument('--cls_reg', type=float, default=0.1, help='reg_coef')
            ...
            with open(filePath, 'a+') as f:
                ...
                print(args)
                from models import DMGIDDC
                embedder = DMGIDDC(args)
                nmi, acc, ari, stdacc, stdnmi, stdari, retxt = embedder.training(f)
                result = "hid_units:{},lr:{},l2_coef:{},cls_reg:{},acc:{},nmi:{},Ari:{},stdnmi:{},stdacc:{},stdari:{}".format(
                    args.hid_units, args.lr, args.l2_coef, args.cls_reg, acc, nmi, ari, stdacc, stdnmi, stdari)
                print(result)
                f.write(retxt)
                f.write('\n')
                f.write(result)
