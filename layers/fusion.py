...
class Mean(_Fusion):
    def __init__(self, n_views, input_sizes):
        ...
        :param input_sizes: Input shapes
        """
        super().__init__(n_views, input_sizes)
...
class WeightedMean(_Fusion):
    ...
    def __init__(self, cfg, input_sizes):
        ...
        self.weights = nn.Parameter(th.full((self.n_views,), 1 / self.n_views), requires_grad=True)
        self.output_size = self.get_weighted_sum_output_size(input_sizes)
...
def get_fusion_module(method_type, n_views, input_sizes):
    return MODULES[method_type](n_views, input_sizes)
