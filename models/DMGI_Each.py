...
class modeler(nn.Module):
    ...
    def forward(self, feature, adj, shuf, sparse, msk, samp_bias1, samp_bias2):
        ...
        pos_reg_loss=0
        neg_reg_loss=0
        for index in range(len(h_1_all)):
            pos_reg_loss = pos_reg_loss+((self.H - h_1_all[index]) ** 2).sum()
            neg_reg_loss = neg_reg_loss+((self.H - h_2_all[index]) ** 2).sum()
