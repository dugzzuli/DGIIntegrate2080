...
class DMGINoCS(embedder):
    ...
    def training(self,f):
        ...
        if(self.args.Fine):
            for epoch in iters:
                ...
                if(epoch%10)==0:
                    ...
                    model.eval()
                    with torch.no_grad():
                        result = model(features, adj, shuf, self.args.sparse, None, None,
                                                   None)  # 解码784，编码10
                        nmi,acc,ari,stdacc,stdnmi,stdari=evaluate(result["h_1_all"].data.detach(), self.idx_train, self.labels, self.args.device)
                    if(accMax<acc):
                        accMax=acc
                        nmiMax=nmi
                        ariMax=ari
        ...
        with torch.no_grad():
            result = model(features, adj, shuf, self.args.sparse, None, None,
                           None)  # 解码784，编码10
            nmi, acc, ari, stdacc, stdnmi, stdari = evaluate(result["h_1_all"].data.detach(), self.idx_train,
                                                             self.labels, self.args.device)
